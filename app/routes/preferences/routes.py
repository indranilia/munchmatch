...
@bp.route("/add_preferences/", methods=[ "POST"])
@token_required
def add_preferences():
    """
    Adding a review
...
def change_range(uuid):
    """
    Update user preferences with new search range.
    ...
    Returns:
    --------
    response (json): A JSON response indicating if the update was successful.
    """
    try:
...
def change_diet(uuid):
    ...
    Returns:
    -----------
    A response object with a message indicating whether the update was successful.
    """
    try:
        reviewData = request.get_json()
...
def change_cuisine(uuid):
    """
    Update the preferred cuisine of the user.
